...
In order to disaggregate the effects of the Contagion and Assistance model parameters, we examine their results separately. Figure XX shows histograms of the scenario-months for Contagion and Assistance, respectively.

Upon visual examination, the distributions with Contagion set to 'On' and 'Off' appear extremely similar. I conduct two statistical tests to determine whether they are in fact drawn from the same underlying distribution -- that is, whether there is no actual difference between them. Both the Kolmogorov-Smirnov  and Mann-Whitney tests indicate that the distributions are in fact different. Nevertheless, the differences are small. The mean ratio for the No Contagion scenario is 1.046, compared to 1.068 for the Contagion scenario. Both a parametric T-Test and nonparametric Wilcoxon Rank-Sum confirm that these means are indeed different, though the difference is small. Similarly, the variance for the No Contagion scenario is 0.045, while the Contagion variance is 0.051. Thus, conflict contagion pushes the scenarios away from the mean, yielding a wider range of outcomes. 

I repeat the same analysis for the the Assistance disabled and enabled scenarios. Again, while the distributions appear extremely similar, both the Kolmogorov-Smirnov and Mann-Whitney tests show that they are different. The mean and variance for Assistance Off are 1.062 and 0.05, respectively, compared to 1.053 and 0.046 when Assistance is enabled. This confirms with our intuition -- international assistance reduces the uncertainty of energy availability, and increases overall energy security, albeit by a small number.
...
Figure XX shows a map of the overall risk of a major supply crisis, with the supply ratio rising above 2. Several things immediately jump out: several countries at the highest risk countries are major net oil exporters, including Canada, Mexico and Iran. While these countries derive a large fraction of their consumption from domestic production, they also rely on imports from abroad 
